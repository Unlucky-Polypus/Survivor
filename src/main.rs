...
async fn main() {
    ...
    sword.texture.set_filter(FilterMode::Nearest);
    draw_texture(&sword.texture, screen_width() / 2.0, screen_height() / 2.0, WHITE);
    ...
    loop {
        ...
        let sword_hitbox = sword.hitbox();

        for ennemy in ennemies.iter_mut() {
            if collision::hitbox_intersects(&sword_hitbox, &ennemy.hitbox()) {
                score += 1;
                ennemy.collided = true;
            }
        }
